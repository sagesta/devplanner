...
/**
 * Find this user's Inbox area, creating it on demand. Single source of truth
 * for the zero-friction quick-capture target. Idempotent: safe to call from
 * any code path that needs to drop a task into the Inbox.
 */
async function ensureInboxArea(userId: string) {
  const existing = await db
    .select()
    .from(areas)
    .where(and(eq(areas.userId, userId), eq(areas.isInbox, true)))
    .limit(1);
  ...
  const [created] = await db
    ...
    .values({
      ...
      icon: "inbox",
      color: "#6B7280",
      // Sort first so it sits at the top of the area list.
      sortOrder: -1,
      isInbox: true,
    })
    .returning();
...
export const areaRoutes = new Hono<AppEnv>()
  .get("/", async (c) => {
    ...
    // Make sure the user has an Inbox so the UI always has a capture target.
    await ensureInboxArea(userId);
    const rows = await db
      .select()
      .from(areas)
  ...
  .get("/inbox", async (c) => {
    const userId = c.get("userId");
    const area = await ensureInboxArea(userId);
    return c.json({ area });
  })
  .post("/", async (c) => {

...
import { materializeNextRecurrence } from "../services/recurrence.js";
...
export const taskRoutes = new Hono<AppEnv>()
  ...
  .post("/bulk-status", async (c) => {
    ...
    // Snapshot only the tasks that might trigger recurrence — those moving
    // *into* "done" from a non-done state and carrying a recurrence rule.
    let priorDoneIds = new Set<string>();
    if (status === "done") {
      const priorRows = await db
        .select({ id: tasks.id, status: tasks.status })
        .from(tasks)
        .where(and(eq(tasks.userId, userId), inArray(tasks.id, taskIds), taskActive));
      priorDoneIds = new Set(priorRows.filter((r) => r.status === "done").map((r) => r.id));
    }
    ...
    for (const row of updated) {
      ...
      if (status === "done" && !priorDoneIds.has(row.id) && row.recurrenceRule) {
        try {
          const newId = await materializeNextRecurrence(db, row.id);
          if (newId) taskCreatedTotal.inc();
        } catch (err) {
          logger.error({ err, taskId: row.id }, "recurrence_materialize_failed");
        }
      }
    }
  ...
  .patch("/:id", async (c) => {
    ...
    const userId = c.get("userId");
    // Snapshot prior status so we can detect the done-transition for
    // recurrence materialization without firing on every PATCH.
    const prior = await db
      .select({ status: tasks.status, recurrenceRule: tasks.recurrenceRule })
      .from(tasks)
      .where(and(eq(tasks.id, id), eq(tasks.userId, userId), taskActive))
      .limit(1);
    ...
    if (v.status === "done" && !wasDone && row.recurrenceRule) {
      try {
        nextRecurrenceId = await materializeNextRecurrence(db, row.id);
        if (nextRecurrenceId) {
          taskCreatedTotal.inc();
        }
      } catch (err) {
    ...
    return c.json({
      task: withTaskApiFields(row),
      ...(nextRecurrenceId ? { nextRecurrenceId } : {}),
    });
  })
  .delete("/:id", async (c) => {
    const idParsed = uuidParam.safeParse(c.req.param("id"));

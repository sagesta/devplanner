...
export async function runMigrations(pool: pg.Pool): Promise<void> {
  ...
  try {
    ...
    // ── areas: is_inbox flag for zero-friction quick capture ───────
    await client.query(`
      ALTER TABLE "areas"
    ...
    // One Inbox per user.
    await client.query(`
      CREATE UNIQUE INDEX IF NOT EXISTS areas_one_inbox_per_user_uidx
        ON areas (user_id) WHERE is_inbox = TRUE;
    `);
